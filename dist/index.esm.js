...
const Button = memo(function Button({ size, mode = "button", state, onChange = NO_OP, onReady = NO_OP }) {
    ...
    useEffect(() => {
        button.current = new Button$1(elementId.current, { size, state, mode });
...
const Sequencer = memo(function Sequencer({ size = [400, 200], mode = "toggle", rows = 5, columns = 10, onChange = () => { }, onStep = () => { }, onReady = () => { } }, ref) {
