...
const Multislider = React__default['default'].memo(function Multislider({ size, max, min, smoothing, step, values, candycane, onChange = NO_OP, onReady = NO_OP }) {
    let multislider = React__default['default'].useRef(null);
    let elementId = React__default['default'].useRef(`nexus-ui-multislider-${getId()}`);
    ...
    return React__default['default'].createElement("div", { id: elementId.current });
});
...
const Sequencer = React.memo(function Sequencer({ size = [400, 200], mode = "toggle", rows = 5, columns = 10, onChange = () => { }, onStep = () => { }, onReady = () => { } }, ref) {
    let sequencer = React.useRef(null);
    let elementId = React.useRef(`nexus-ui-sequencer-${getId()}`);
    React.useEffect(() => {
        window.Nexus = Nexus__namespace;
